...
export default function NotificationsPage() {
  ...
  const handleDeleteAll = async () => {
    if (!confirm(t('dashboard.doctor.notificationsPage.confirmDeleteAll'))) return;
    const token = localStorage.getItem('access_token') || '';
    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="mb-4 flex gap-1 rounded-lg bg-gray-100 p-1">
          {(['all', 'unread', 'read'] as FilterStatus[]).map((f) => (
            ...
            >
              {t(`dashboard.doctor.notificationsPage.${f}`)}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {loading ? (
            ...
            <div className="rounded-lg bg-white p-8 text-center shadow">
              <p className="text-gray-400">{t('dashboard.doctor.notificationsPage.noNotifications')}</p>
            </div>
          ) : (
            items.map((n) => (
              <div
              ...
              >
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-gray-900">{n.type}</span>
                      {n.patient_name && (
                        <span className="text-sm text-gray-500">- {n.patient_name}</span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-2 ml-4">
                  {!n.is_read && (
                    <button onClick={() => handleMarkRead(n.sid)}
                      className="rounded p-1 text-blue-500 hover:bg-blue-50" title="Mark as read">
                      <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                      </svg>
                    </button>
                  )}
                  <button onClick={() => handleDelete(n.sid)}
