...
import { doctorPayments, DoctorPaymentItem, DoctorPaymentStats } from '@/lib/api';
...
export default function PaymentsPage() {
  ...
  const [stats, setStats] = useState<DoctorPaymentStats | null>(null);
  ...
  useEffect(() => {
    if (user) {
      ...
      doctorPayments.stats(token).then(setStats).catch(() => {});
    }
  }, [user]);
  ...
  useEffect(() => {
    if (user) {
      setLoading(true);
      const token = localStorage.getItem('access_token') || '';
      doctorPayments.list(token, page)
        .then((data) => {
    ...
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-lg bg-white p-5 shadow">
          ...
          <p className="text-2xl font-bold text-gray-900">${stats?.this_month_revenue ?? 0}</p>
        </div>
        <div className="rounded-lg bg-white p-5 shadow">
            ...
            <tbody className="divide-y">
              {payments.map((p) => (
                <tr key={p.sid} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600">{methodLabels[p.payment_method] || p.payment_method}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-1 text-xs font-medium ${statusColors[p.status] || ''}`}>
                      {p.status}
                    </span>
                  </td>
                </tr>
