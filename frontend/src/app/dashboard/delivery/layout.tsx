...
import { Menu, Home, History, User, LogOut } from 'lucide-react';

const navItems = [
  { key: 'dashboard', href: '/dashboard/delivery', icon: Home, exact: true },
  { key: 'history', href: '/dashboard/delivery/history', icon: History },
  { key: 'profile', href: '/dashboard/delivery/profile', icon: User },
];
                  ...
                  return (
                    <Link key={item.key} href={item.href} onClick={() => setOpen(false)}>
                      <Button variant="ghost" className={cn('w-full justify-start gap-3', isActive && 'bg-primary/10 text-primary')}>
                        ...
                        {t(`dashboard.delivery.nav.${item.key}`)}
                      </Button>
                    </Link>
                  );
                })}
